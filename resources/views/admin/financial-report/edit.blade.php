<x-layout>
    <x-slot:title>Edit Laporan Keuangan</x-slot:title>

    <div class="max-w-4xl mx-auto py-6">
        <h2 class="text-2xl font-bold text-white mb-6">Edit Laporan Keuangan</h2>
        ...
        <form action="{{ route('admin.financial-reports.update', $financialReport) }}" method="POST" enctype="multipart/form-data"
            ...
            @if($financialReport->file_path)
                <div>
                    <label class="block text-white mb-2 font-medium">Current PDF File</label>
                    <a href="{{ Storage::url($financialReport->file_path) }}" target="_blank"
            ...
            <div>
                ...
                <select name="report_type" id="report_type" required onchange="togglePeriodFields()"
                    ...
                    <option value="monthly" {{ old('report_type', $financialReport->report_type) === 'monthly' ? 'selected' : '' }}>Bulanan</option>
                    <option value="quarterly" {{ old('report_type', $financialReport->report_type) === 'quarterly' ? 'selected' : '' }}>Kuartalan</option>
                    <option value="yearly" {{ old('report_type', $financialReport->report_type) === 'yearly' ? 'selected' : '' }}>Tahunan</option>
                </select>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="year" value="{{ old('year', $financialReport->year) }}" required
                        min="2000" max="2100"
                        class="w-full bg-gray-700 text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                ...
                <div id="month_field" class="hidden">
                    ...
                    <select name="month"
                        ...
                        @foreach(range(1, 12) as $m)
                            <option value="{{ $m }}" {{ old('month', $financialReport->month) == $m ? 'selected' : '' }}>
                ...
                <div id="quarter_field" class="hidden">
                    ...
                    <select name="quarter"
                        ...
                        <option value="1" {{ old('quarter', $financialReport->quarter) == 1 ? 'selected' : '' }}>Q1 (Jan-Mar)</option>
                        <option value="2" {{ old('quarter', $financialReport->quarter) == 2 ? 'selected' : '' }}>Q2 (Apr-Jun)</option>
                        <option value="3" {{ old('quarter', $financialReport->quarter) == 3 ? 'selected' : '' }}>Q3 (Jul-Sep)</option>
                        <option value="4" {{ old('quarter', $financialReport->quarter) == 4 ? 'selected' : '' }}>Q4 (Oct-Dec)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" name="income" value="{{ old('income', $financialReport->income) }}" required
                        min="0" step="0.01"
                        class="w-full bg-gray-700 text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="expense" value="{{ old('expense', $financialReport->expense) }}" required
                        min="0" step="0.01"
                        class="w-full bg-gray-700 text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $financialReport->order) }}"
                        class="w-full bg-gray-700 text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                    ...
                    class="w-full bg-gray-700 text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $financialReport->description) }}</textarea>
            </div>

            <!-- File Upload -->
            <div>
                <label class="block text-white mb-2 font-medium">Upload File PDF Baru (optional)</label>
                <input type="file" name="file" accept=".pdf"
                    class="w-full bg-gray-700 text-white rounded px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                <p class="text-gray-400 text-sm mt-1">Kosongkan jika tidak ingin mengganti file. Max: 5MB</p>
            </div>

            <!-- Published Status -->
            <div class="p-4 bg-indigo-500/10 border border-indigo-500 rounded">
                <label class="flex items-center text-white">
                    <input type="checkbox" name="is_published" value="1" {{ $financialReport->is_published ? 'checked' : '' }}
    ...
    <script>
        ...
        // Run on page load to handle current values
