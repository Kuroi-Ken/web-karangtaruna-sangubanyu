<x-layout>
    <x-slot:title>Manage Financial Reports</x-slot:title>

    <div class="py-6">
        <x-admin-menu />
        
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-white">Laporan Keuangan Management</h2>
        ...
        <div class="bg-gray-800 rounded-lg overflow-hidden">
            <table class="w-full">
                <thead class="bg-gray-700">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">Periode</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">Judul</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-300 uppercase">Pemasukan</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-300 uppercase">Pengeluaran</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-300 uppercase">Saldo</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    @forelse ($reports as $report)
                        <tr class="hover:bg-gray-700/50">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-white">{{ $report->title }}</div>
                                @if($report->file_path)
                                    <a href="{{ Storage::url($report->file_path) }}" target="_blank" 
                                       class="text-xs text-indigo-400 hover:underline flex items-center gap-1 mt-1">
                            ...
                            <td class="px-6 py-4 text-right text-green-400 font-medium">
                                {{ $report->formatted_income }}
                            </td>
                            <td class="px-6 py-4 text-right text-red-400 font-medium">
                                {{ $report->formatted_expense }}
                            </td>
                            <td class="px-6 py-4 text-right font-bold {{ $report->balance >= 0 ? 'text-green-400' : 'text-red-400' }}">
